...
fn spawn_camera(game: &mut ResMut<Game>, commands: &mut Commands) {
...
fn setup_camera_system(game: ResMut<Game>, mut query: Query<&mut FlyCamera>) {
...
fn spawn_ground(game: &mut ResMut<Game>, commands: &mut Commands) {
...
fn spawn_vehicle(
...

    {
        ...
        let (rf_axle_joint, rf_wheel_joint, rf_wheel) = spawn_attached_wheel(
            "RF".to_string(),
            Tag::FrontWheel,
            body,
            body_pos,
            offset,
            vehicle_cfg.front_hh,
            vehicle_cfg.front_r,
            &mut commands,
        );
        (game.rf_axle_joint, game.rf_wheel_joint, game.rf_wheel) =
            (Some(rf_axle_joint), Some(rf_wheel_joint), Some(rf_wheel));
        println!("rf_wheel Entity ID {:?}", rf_wheel);
    }

    if true {
        let offset = Vec3::new(-x_off, -y_off, z_off);
        let (lf_axle_joint, lf_wheel_joint, lf_wheel) = spawn_attached_wheel(
            "LF".to_string(),
            Tag::FrontWheel,
            body,
            body_pos,
        ...
        (game.lf_axle_joint, game.lf_wheel_joint, game.lf_wheel) =
            (Some(lf_axle_joint), Some(lf_wheel_joint), Some(lf_wheel));
        println!("lf_wheel Entity ID {:?}", lf_wheel);
    }

    if true {
        let offset = Vec3::new(x_off, -y_off, -z_off);
        let (rr_axle_joint, rr_wheel_joint, rr_wheel) = spawn_attached_wheel(
            "RR".to_string(),
            Tag::RearWheel,
            body,
            body_pos,
        ...
        (game.rr_axle_joint, game.rr_wheel_joint, game.rr_wheel) =
            (Some(rr_axle_joint), Some(rr_wheel_joint), Some(rr_wheel));
        println!("rr_wheel Entity ID {:?}", rr_wheel);
    }

    if true {
        let offset = Vec3::new(-x_off, -y_off, -z_off);
        let (lr_axle_joint, lr_wheel_joint, lr_wheel) = spawn_attached_wheel(
            "LR".to_string(),
            Tag::RearWheel,
            body,
            body_pos,
        ...
        (game.lr_axle_joint, game.lr_wheel_joint, game.lr_wheel) =
            (Some(lr_axle_joint), Some(lr_wheel_joint), Some(lr_wheel));
...
fn spawn_attached_wheel(
    ...
    let axle_joint = spawn_axle_joint(
        body,
        axle,
        point![anchor1.x, anchor1.y, anchor1.z],
        point![anchor2.x, anchor2.y, anchor2.z],
        &mut commands,
    );
...
fn motor_steer(
    ...
    joint.data = joint
        .data
        .motor_position(JointAxis::AngX, angle_rad, stiffness, damping)
...
fn camera_collision_system(
    mut query: Query<(&FlyCamera, &Transform, &mut ColliderPositionComponent)>,
) {
...
fn display_events_system(
    ...
    for intersection_event in intersection_events.iter() {
        println!(
            "Received intersection event: collider1 {:?} collider2 {:?}",
    ...
    for contact_event in contact_events.iter() {
        match contact_event {
            ContactEvent::Started(collider1, collider2) => println!(
                "Received contact START event: collider1 {:?} collider2 {:?}",
                collider1.entity(),
                collider2.entity()
            ),
            ContactEvent::Stopped(collider1, collider2) => println!(
                "Received contact STOP event: collider1 {:?} collider2 {:?}",
                collider1.entity(),
                collider2.entity()
            ),
        }
...
fn set_cylinder_hh(mut coll_shape: &mut ColliderShapeComponent, new_hh: f32) {
    let shape = coll_shape.make_mut();
    let mut cylinder = shape.as_cylinder_mut().unwrap();
    cylinder.half_height = new_hh;
}

fn set_cylinder_r(mut coll_shape: &mut ColliderShapeComponent, new_r: f32) {
    let shape = coll_shape.make_mut();
    let mut cylinder = shape.as_cylinder_mut().unwrap();
...
fn draw_density_param_ui(
    ...
    match &mut mass_props_coll as &mut ColliderMassProps {
        ColliderMassProps::Density(density) => {
            if ui
                .add(Slider::new(&mut *density, 0.01..=1000.0).text(format!("{} Density", name)))
                .changed()
            {
            ...
            egui::CollapsingHeader::new("Wheel sizes")
                ...
                .show(ui, |ui| {
                    ui.vertical(|ui| {
                        ...
                        ui.add(Slider::new(&mut cylinder.radius, 0.05..=1.0).text(label));

                        let label = format!("{} half height", cylinder.half_height);
                        ui.add(Slider::new(&mut cylinder.half_height, 0.05..=1.0).text(label));
                    }); // ui.vertical
                }); // ui.collapsing
        }
        _ => (),
...
fn draw_body_params_ui_collapsing(
    ...
    ui.collapsing(section_name, |ui| {
        ui.vertical(|ui| {
            ...
            ui.add(Slider::new(&mut cuboid.half_extents[0], 0.05..=5.0).text(label));

            let label = format!("{} half height Y", cuboid.half_extents[1]);
            ui.add(Slider::new(&mut cuboid.half_extents[1], 0.05..=5.0).text(label));

            let label = format!("{} half height Z", cuboid.half_extents[2]);
            ui.add(Slider::new(&mut cuboid.half_extents[2], 0.05..=5.0).text(label));
        }); // ui.vertical
    }); // ui.collapsing
}
    ...
    let out = window.show(ui_context.ctx_mut(), |ui| {
        ...
        ui.collapsing("Rear Wheels".to_string(), |ui| {
            ui.vertical(|ui| {
                ...
                rear_wh_r_changed = ui
                    .add(
                        Slider::new(&mut vehicle_cfg.rear_r, 0.05..=1.0).text("Rear wheels radius"),
                    )
                    .changed();
            }); // ui.vertical
        }); // ui.collapsing
        ...
        for (mass_props_coll, mass_props_rbody, mut coll_shape, name_comp, tag) in query.iter_mut()
        {
            ...
            if name.starts_with("RF") {
                ...
                draw_body_params_ui_collapsing(
                    ui,
                    name,
                    &mut mass_props_coll,
                    &mut mass_props_rbody,
